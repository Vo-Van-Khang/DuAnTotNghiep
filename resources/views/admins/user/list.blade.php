					...
					<table class="main__table">
						...
						<tbody>
							...
							<tr class="tr__remove" id_remove="{{$user->id}}">
								...
								<td>
                                    @if ($user->status == 0)
										<div @class(['main__table-text main__table-text--red', 'status__item__update' => $user->role !== 'admin'])>Bị cấm</div>
                                    @else
										<div @class(['main__table-text main__table-text--green', 'status__item__update' => $user->role !== 'admin'])>Bình thường</div>
                                    @endif
								</td>
								<td>
                                    @if ($user->premium == 0)
